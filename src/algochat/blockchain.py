...
and indexers. Implementations can use any Algorand SDK (e.g., py-algorand-sdk).
"""
...
class AlgodClient(ABC):
    ...
    @abstractmethod
    async def wait_for_confirmation(self, txid: str, rounds: int = 10) -> TransactionInfo:
...
class IndexerClient(ABC):
    ...
    @abstractmethod
    async def wait_for_indexer(self, txid: str, timeout_secs: int = 30) -> NoteTransaction:
        """Wait for a transaction to be indexed."""
        ...


def parse_key_announcement(note: bytes, address: str) -> Optional[DiscoveredKey]:
    """
    ...
    Args:
        note: The transaction note field
        address: The sender address (for verification context)

    Returns:
        DiscoveredKey if valid, None otherwise
    """
    if len(note) < 32:
        return None

    public_key = bytes(note[:32])
    is_verified = False
...
async def discover_encryption_key(
    ...
    Args:
        ...
        address: The Algorand address to find the key for

    Returns:
        DiscoveredKey if found, None otherwise
    """
    ...
    for tx in transactions:
        ...
        key = parse_key_announcement(tx.note, address)
        if key is not None:
            return key
